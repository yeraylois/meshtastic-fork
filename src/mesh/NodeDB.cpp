...
uint32_t
read8(uint8_t bits, uint8_t dummy, uint8_t cs, uint8_t sck, uint8_t mosi, uint8_t dc, uint8_t rst) {
  uint32_t ret    = 0;
  uint8_t  SDAPIN = mosi;
...
void write9(
    uint8_t val, uint8_t dc_val, uint8_t cs, uint8_t sck, uint8_t mosi, uint8_t dc, uint8_t rst) {
  pinMode(mosi, OUTPUT);
...
uint32_t readwrite8(uint8_t cmd,
                    uint8_t bits,
                    uint8_t dummy,
                    uint8_t cs,
                    uint8_t sck,
...
meshtastic_CriticalErrorCode error_code =
    meshtastic_CriticalErrorCode_NONE;  // For the error code, only show values from this boot
                                        // (discard value from flash)
...
NodeDB::NodeDB() {
  ...
  esp_err_t err =
      esp_efuse_read_field_blob(ESP_EFUSE_OPTIONAL_UNIQUE_ID, unique_id, sizeof(unique_id) * 8);
  if (err == ESP_OK) {
  ...
  memcpy(
      myNodeInfo.device_id.bytes + sizeof(device_id_start), &device_id_end, sizeof(device_id_end));
  myNodeInfo.device_id.size = 16;
  ...
  myNodeInfo.min_app_version =
      30200;  // format is Mmmss (where M is 1+the numeric major number. i.e. 30200 means 2.2.00
  // Note! We do this after loading saved settings, so that if somehow an invalid nodenum was stored
  // in preferences we won't keep using that nodenum forever. Crummy guess at our nodenum (but we
  // will check against the nodedb to avoid conflicts)
  pickNewNodeNum();
  ...
  if (config.security.private_key.size == 32 && config.security.public_key.size == 32) {
    ...
    memcpy(
        owner.public_key.bytes, config.security.public_key.bytes, config.security.public_key.size);
  ...
  // If we are setup to broadcast on the default channel, ensure that the telemetry intervals are
  // coerced to the minimum value of 30 minutes or more
  ...
  moduleConfig.neighbor_info.update_interval = Default::getConfiguredOrMinimumValue(
...
/**
 * Most (but not always) of the time we want to treat packets 'from' the local phone (where from ==
 * 0), as if they originated on the local node. If from is zero this function returns our node
 * number instead
 */
...
bool NodeDB::factoryReset(bool eraseBleBonds) {
  ...
  installDefaultConfig(
      !eraseBleBonds);  // Also preserve the private key if we're not erasing BLE bonds
...
void NodeDB::installDefaultConfig(bool preserveKey = false) {
  ...
  bool    shouldPreserveKey =
      preserveKey && config.has_security && config.security.private_key.size > 0;
...
#if (defined(T_DECK) || defined(T_WATCH_S3) || defined(UNPHONE) || defined(PICOMPUTER_S3)          \
     || defined(SENSECAP_INDICATOR) || defined(ELECROW_PANEL))                                     \
    && HAS_TFT
...
#if defined(ST7735_CS) || defined(USE_EINK) || defined(ILI9341_DRIVER) || defined(ILI9342_DRIVER)  \
...
void NodeDB::installDefaultModuleConfig() {
...
#ifdef HAS_I2S
  // Don't worry about the other settings for T-Watch, we'll also use the DRV2056 behavior for
  // notifications
  moduleConfig.external_notification.enabled              = true;
  ...
  config.bluetooth.enabled = false;  // BT = OFF

  // DISPLAY CONFIG
  config.display.screen_on_secs = 1;  // SCREEN OFF AFTER 1 SEC
  ...
  for (int i = 0; i < numMeshNodes; i++) {
    if (meshNodes->at(i).has_user) {
      if (meshNodes->at(i).user.public_key.size > 0) {
        if (memfll(
                meshNodes->at(i).user.public_key.bytes, 0, meshNodes->at(i).user.public_key.size)) {
...
void NodeDB::installDefaultDeviceState() {
  ...
  snprintf(
      owner.long_name, sizeof(owner.long_name), (const char*) USERPREFS_CONFIG_OWNER_LONG_NAME);
#else
  snprintf(owner.long_name, sizeof(owner.long_name), "Meshtastic %04x", getNodeNum() & 0x0ffff);
#endif
#ifdef USERPREFS_CONFIG_OWNER_SHORT_NAME
  snprintf(
      owner.short_name, sizeof(owner.short_name), (const char*) USERPREFS_CONFIG_OWNER_SHORT_NAME);
#else
  snprintf(owner.short_name, sizeof(owner.short_name), "%04x", getNodeNum() & 0x0ffff);
#endif
  snprintf(
      owner.id, sizeof(owner.id), "!%08x", getNodeNum());  // Default node ID now based on nodenum
  memcpy(owner.macaddr, ourMacAddr, sizeof(owner.macaddr));
  owner.has_is_unmessagable = true;
...
void NodeDB::pickNewNodeNum() {
  ...
  while (((found = getMeshNode(nodeNum))
    ...
    if (found)
      LOG_WARN("NOTE! Our desired nodenum 0x%x is invalid or in use, by MAC ending in 0x%02x%02x "
               "vs our 0x%02x%02x, so "
               "trying for 0x%x",
               nodeNum,
               found->user.macaddr[4],
               found->user.macaddr[5],
               ourMacAddr[4],
               ourMacAddr[5],
               candidate);
    nodeNum = candidate;
...
LoadFileResult NodeDB::loadProto(const char*         filename,
                                 size_t              protoSize,
                                 size_t              objSize,
...
void NodeDB::loadFromDisk() {
  // Mark the current device state as completely unusable, so that if we fail reading the entire
  // file from disk we will still factoryReset to restore things.
  ...
  if (FSCom.exists(legacyPrefFileName)) {
    ...
    if (loadProto(configFileName,
                  meshtastic_LocalConfig_size,
                  sizeof(meshtastic_LocalConfig),
...
#endif
  auto state = loadProto(nodeDatabaseFileName,
                         getMaxNodesAllocatedSize(),
  ...
  // static DeviceState scratch; We no longer read into a tempbuf because this structure is 15KB of
  // valuable RAM
  state = loadProto(deviceStateFileName,
                    meshtastic_DeviceState_size,
  ...
  // meshtastic_DeviceState.  Because most of our critical config may still be valid (in the other
  // files - loaded next). Also, if we did fail on reading we probably failed on the enormous (and
  // non critical) nodeDB.  So DO NOT install default device state. if (state !=
  // LoadFileResult::LOAD_SUCCESS) {
  ...
  state = loadProto(configFileName,
                    meshtastic_LocalConfig_size,
                    sizeof(meshtastic_LocalConfig),
                    &meshtastic_LocalConfig_msg,
                    &config);
  ...
  // Initialize admin_key_count to zero
  byte numAdminKeys = 0;
#if defined(USERPREFS_USE_ADMIN_KEY_0) || defined(USERPREFS_USE_ADMIN_KEY_1)                       \
    || defined(USERPREFS_USE_ADMIN_KEY_2)
  uint16_t sum = 0;
#endif
#ifdef USERPREFS_USE_ADMIN_KEY_0
  ...
  state = loadProto(moduleConfigFileName,
                    meshtastic_LocalModuleConfig_size,
  ...
  state = loadProto(channelFileName,
                    meshtastic_ChannelFile_size,
                    sizeof(meshtastic_ChannelFile),
                    &meshtastic_ChannelFile_msg,
                    &channelFile);
  if (state != LoadFileResult::LOAD_SUCCESS) {
  ...
  state = loadProto(uiconfigFileName,
  ...
  if (settingsMap[has_configDisplayMode]) {
    config.display.displaymode =
        (_meshtastic_Config_DisplayConfig_DisplayMode) settingsMap[configDisplayMode];
  }
...
#endif
  return saveProto(
      channelFileName, meshtastic_ChannelFile_size, &meshtastic_ChannelFile_msg, &channelFile);
}
...
bool NodeDB::saveDeviceStateToDisk() {
  ...
  // Because so huge we _must_ not use fullAtomic, because the filesystem is probably too small to
  // hold two copies of this
  return saveProto(deviceStateFileName,
                   meshtastic_DeviceState_size,
                   &meshtastic_DeviceState_msg,
                   &devicestate,
                   true);
...
bool NodeDB::saveNodeDatabaseToDisk() {
  ...
  return saveProto(
      nodeDatabaseFileName, nodeDatabaseSize, &meshtastic_NodeDatabase_msg, &nodeDatabase, false);
}
...
bool NodeDB::saveToDiskNoRetry(int saveWhat) {
  ...
  if (saveWhat & SEGMENT_CONFIG) {
    ...
    success &= saveProto(
        configFileName, meshtastic_LocalConfig_size, &meshtastic_LocalConfig_msg, &config);
  }
  ...
  if (saveWhat & SEGMENT_MODULECONFIG) {
    ...
    success &= saveProto(moduleConfigFileName,
                         meshtastic_LocalModuleConfig_size,
                         &meshtastic_LocalModuleConfig_msg,
                         &moduleConfig);
  }
...
#ifdef ARCH_NRF52  // @geeksville is not ready yet to say we should do this on other platforms.  See
                   // bug #4184 discussion
    spiLock->lock();
    FSCom.format();
    spiLock->unlock();
...
void NodeDB::updatePosition(uint32_t nodeId, const meshtastic_Position& p, RxSource src) {
  ...
  if (src == RX_SRC_LOCAL) {
    ...
    // A lot of position reports don't have time populated.  In that case, be careful to not blow
    // away the time we recorded based on the packet rxTime
    //
...
bool NodeDB::updateUser(uint32_t nodeId, meshtastic_User& p, uint8_t channelIndex) {
  ...
  if (p.public_key.size == 32 && nodeId != nodeDB->getNodeNum()) {
    ...
    if (owner.public_key.size == 32 && memcmp(p.public_key.bytes, owner.public_key.bytes, 32) == 0
      ...
      char warning[]  = "Remote device %s has advertised your public key. This may indicate a "
                        "compromised key. You may need "
  ...
  if (info->user.public_key.size
      > 0) {  // if we have a key for this user already, don't overwrite with a new one
  ...
  if (nodeId != getNodeNum())
    info->channel = channelIndex;  // Set channel we need to use to reach this node (but don't set
                                   // our own channel)
  LOG_DEBUG("Update changed=%d user %s/%s, id=0x%08x, channel=%d",
            changed,
            info->user.long_name,
...
void NodeDB::updateFrom(const meshtastic_MeshPacket& mp) {
  ...
  if (mp.which_payload_variant == meshtastic_MeshPacket_decoded_tag && mp.from) {
    ...
    // If hopStart was set and there wasn't someone messing with the limit in the middle, add
    // hopsAway
    ...
    while (changed) {  // dumb reverse bubble sort, but probably not bad for what we're doing
      ...
      for (int i = numMeshNodes - 1; i > 0; i--) {  // lowest case this should examine is i == 1
        if (meshNodes->at(i - 1).num == getNodeNum()) {
          // noop
        } else if (meshNodes->at(i).num == getNodeNum()) {  // in the oddball case our own node num
                                                            // is not at location 0, put it there
...
meshtastic_NodeInfoLite* NodeDB::getOrCreateMeshNode(NodeNum n) {
  ...
  if (!lite) {
    if (isFull()) {
      LOG_INFO("Node database full with %i nodes and %u bytes free. Erasing oldest entry",
               numMeshNodes,
               memGet.getFreeHeap());
...
bool NodeDB::restorePreferences(meshtastic_AdminMessage_BackupLocation location, int restoreWhat) {
  ...
  if (location == meshtastic_AdminMessage_BackupLocation_FLASH) {
    ...
    meshtastic_BackupPreferences backup = meshtastic_BackupPreferences_init_zero;
    success                             = loadProto(backupFileName,
                        meshtastic_BackupPreferences_size,
                        sizeof(meshtastic_BackupPreferences),
                        &meshtastic_BackupPreferences_msg,
                        &backup);
    if (success) {
...
void recordCriticalError(meshtastic_CriticalErrorCode code,
                         uint32_t                     address,
                         const char*                  filename) {
  if (filename) {
  ...
  // Currently portuino is mostly used for simulation.  Make sure the user notices something really
  // bad happened
#ifdef ARCH_PORTDUINO
