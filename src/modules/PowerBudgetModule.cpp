...
#define PB_WIRE Wire
#else
#error "Unsupported board: define BOARD_HELTEC_WIRELESS_STICK_V3 or BOARD_HELTEC_MESH_NODE_T114_V2_0"
#endif
...
PowerBudgetModule::PowerBudgetModule()
    : SinglePortModule("power_budget", kPort), concurrency::OSThread("power_budget")
{
...
void PowerBudgetModule::printHeaderOnce()
{
    ...
    PB_LOGI("WS3 (ESP32-S3): DEDICATED I2C BUS = Wire1  SDA=%d  SCL=%d  addr=0x%02X\n",
            (int)PB_I2C_SDA, (int)PB_I2C_SCL, (unsigned)PB_INA_ADDR);
#elif defined(BOARD_HELTEC_MESH_NODE_T114_V2_0)
    ...
    PB_LOGI("CADENCE: Sample=%u ms  Live=%u ms  Summary=%u ms\n",
            (unsigned)PB_SAMPLE_MS, (unsigned)PB_PRINT_MS, (unsigned)PB_SUMMARY_MS);
    PB_LOGI("BATTERY MODEL: C=%.0f mAh  V=%.2f V  Eff=%.0f%%\n",
            (double)PB_BATT_CAP_MAH, (double)PB_VBAT_NOM_V, (double)(PB_REG_EFF * 100.0f));
    PB_LOGI("\n=======================================================\n");
}
