...
T114Rs485SlaveModule::T114Rs485SlaveModule()
  : SinglePortModule("t114_rs485_slave", kPort)
  , concurrency::OSThread("t114_rs485")
{
...
void T114Rs485SlaveModule::initOnce() {
  ...
  T114_LOGI("RS485 init: baud=%lu RX=%d TX=%d DIR=%d, NODE_ID=%d\n",
            (unsigned long)RS485_BAUD, RS485_PIN_RX, RS485_PIN_TX, RS485_PIN_DIR, NODE_ID);
}
...
uint8_t T114Rs485SlaveModule::computeXOR(const char* s, size_t n) {
  uint8_t cs = 0;
  for (size_t i = 0; i < n; ++i) cs ^= (uint8_t)s[i];
  return cs;
}
...
void T114Rs485SlaveModule::handleLine(const String& line) {
  ...
  if (comma < 0 || star < 0 || star <= comma + 1) return;
...
void T114Rs485SlaveModule::handleRx() {
  while (Serial1.available()) {
    ...
    if (ch == '\n' || ch == '\r') {
      ...
      if (rxBuf_.length() < 240) rxBuf_ += ch;
      else rxBuf_.remove(0); 
    }
  }
}
...
int32_t T114Rs485SlaveModule::runOnce() {
  ...
  if (now - tLastHB_ >= HB_INTERVAL_MS) {
    ...
    if (L < 0) L = 0;
    if ((size_t)L > sizeof(payload)) L = sizeof(payload);
    ...
    if (F < 0) F = 0;
    if ((size_t)F > sizeof(frame)) F = sizeof(frame);
