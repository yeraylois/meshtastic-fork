...
TrafficRs485CoordinatorModule::TrafficRs485CoordinatorModule()
  : SinglePortModule("traffic_rs485", kPort)
  , concurrency::OSThread("TrafficRs485Coordinator")
{
}
...
{
  if (RS485_LED_RED_PIN   >= 0) digitalWrite(RS485_LED_RED_PIN,   r ? HIGH : LOW);
  if (RS485_LED_AMBER_PIN >= 0) digitalWrite(RS485_LED_AMBER_PIN, a ? HIGH : LOW);
  if (RS485_LED_GREEN_PIN >= 0) digitalWrite(RS485_LED_GREEN_PIN, g ? HIGH : LOW);
}
...
void TrafficRs485CoordinatorModule::setupSignals_()
{
  ...
  vR_[VM_S2N] = RS485_V_S2N_R_PIN; vA_[VM_S2N] = RS485_V_S2N_A_PIN; vG_[VM_S2N] = RS485_V_S2N_G_PIN;
  vR_[VM_S2W] = RS485_V_S2W_R_PIN; vA_[VM_S2W] = RS485_V_S2W_A_PIN; vG_[VM_S2W] = RS485_V_S2W_G_PIN;
  vR_[VM_N2S] = RS485_V_N2S_R_PIN; vA_[VM_N2S] = RS485_V_N2S_A_PIN; vG_[VM_N2S] = RS485_V_N2S_G_PIN;
  vR_[VM_N2W] = RS485_V_N2W_R_PIN; vA_[VM_N2W] = RS485_V_N2W_A_PIN; vG_[VM_N2W] = RS485_V_N2W_G_PIN;
  vR_[VM_W2N] = RS485_V_W2N_R_PIN; vA_[VM_W2N] = RS485_V_W2N_A_PIN; vG_[VM_W2N] = RS485_V_W2N_G_PIN;
  vR_[VM_W2S] = RS485_V_W2S_R_PIN; vA_[VM_W2S] = RS485_V_W2S_A_PIN; vG_[VM_W2S] = RS485_V_W2S_G_PIN;

  /* PEDESTRIAN PINMAPS */
  pR_[PX_N1] = RS485_P_N1_R_PIN; pG_[PX_N1] = RS485_P_N1_G_PIN;
  pR_[PX_S1] = RS485_P_S1_R_PIN; pG_[PX_S1] = RS485_P_S1_G_PIN;
  pR_[PX_W2] = RS485_P_W2_R_PIN; pG_[PX_W2] = RS485_P_W2_G_PIN;
  pR_[PX_S2] = RS485_P_S2_R_PIN; pG_[PX_S2] = RS485_P_S2_G_PIN;
  pR_[PX_N2] = RS485_P_N2_R_PIN; pG_[PX_N2] = RS485_P_N2_G_PIN;
  pR_[PX_W1] = RS485_P_W1_R_PIN; pG_[PX_W1] = RS485_P_W1_G_PIN;
    ...
    if (c == '\n' || c == '\r') {
      if (rxLen_ > 0) {
        ...
        while (rxLen_ && (rxBuf_[rxLen_-1] == '\r' || rxBuf_[rxLen_-1] == ' ' || rxBuf_[rxLen_-1] == '\t'))
          rxBuf_[--rxLen_] = '\0';
        if (rxLen_) handleLine(rxBuf_);
        rxLen_ = 0;
      }
      continue;
    }
    if (rxLen_ < RX_MAX) rxBuf_[rxLen_++] = c;
    else rxLen_ = 0; /* OVERFLOW -> DROP LINE */
  }
}
...
void TrafficRs485CoordinatorModule::scheduleElectionBackoff_()
{
  ...
  electBackoffUntilMs_ = millis()
                       + RS485_ELECT_BACKOFF_BASE_MS
                       + (uint32_t)r * RS485_ELECT_BACKOFF_STEP_MS
                       + jitter;
...
void TrafficRs485CoordinatorModule::followerTick_()
{
  ...
  LOG_DEBUG("local lights: safety=%d, amber=%d, allred=%d, case=%u, offNode=%u leader=%u isLeader=%d\n",
