...
Ws3OptoPMModule::Ws3OptoPMModule()
  : SinglePortModule("opto_pm_ws3", kPort)
  , concurrency::OSThread("opto_pm_ws3")
{
...
void Ws3OptoPMModule::initOnce() {
  ...
  WS3_OPTO_LOGI("▶ Monitoring power via PC817 (pin=%d, led=%d, pullup=%u, deb=%u)\n",
                (int)WS3_OPTO_PM_PIN, (int)WS3_OPTO_PM_LED,
                (unsigned)WS3_OPTO_PM_PULLUP, (unsigned)WS3_OPTO_PM_DEBOUNCE_MS);
