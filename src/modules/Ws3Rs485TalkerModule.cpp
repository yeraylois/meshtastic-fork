...
Ws3Rs485TalkerModule::Ws3Rs485TalkerModule()
  : SinglePortModule("ws3_rs485_master", kPort)
  , concurrency::OSThread("ws3_rs485")
{
  WS3RS_LOGI("CONSTRUCTOR OF Ws3Rs485TalkerModule\n");
}
...
void Ws3Rs485TalkerModule::initOnce() {
  ...
  WS3RS_LOGI("RS485 init: baud=%lu RX=GPIO%d TX=GPIO%d DIR=GPIO%d | LED(R,A,G)=(%d,%d,%d)\n",
             ...
             WS3_RS485_PIN_RX, WS3_RS485_PIN_TX, WS3_RS485_PIN_DIR,
             WS3_LED_RED_PIN, WS3_LED_AMBER_PIN, WS3_LED_GREEN_PIN);
}
...
void Ws3Rs485TalkerModule::applyCaseToMaster(uint8_t c) {
  ...
  if (c == 2) digitalWrite(WS3_LED_GREEN_PIN, HIGH); // MASTER: GREEN
  else        digitalWrite(WS3_LED_RED_PIN,   HIGH); // MASTER: RED
}
...
void Ws3Rs485TalkerModule::handleLine(const char* lineZ) {
  // WAIT "H,<id>,<cnt>*CS"
  if (strncmp(lineZ, "H,", 2) != 0) return;

  const char* p1 = strchr(lineZ + 2, ',');     if (!p1) return;
  const char* p2 = strchr(p1 + 1, ',');        (void)p2; // reservado si decides usar el campo
  const char* star = strchr(p1 + 1, '*');      if (!star) return;
  ...
  if (n1 >= sizeof(tmp)) return;
  memcpy(tmp, lineZ + 2, n1); tmp[n1] = '\0';
  uint8_t id = (uint8_t) strtoul(tmp, nullptr, 10);

  // CNT
  size_t n2 = (size_t)(star - (p1 + 1));
  if (n2 >= sizeof(tmp)) return;
  memcpy(tmp, p1 + 1, n2); tmp[n2] = '\0';
...
int32_t Ws3Rs485TalkerModule::runOnce() {
  ...
  if (!inAmberPhase_ && (now - tLastCase_) >= RS_CASE_INTERVAL_MS) {
    ...
    if (KA > 0) sendFrame(aBuf, (size_t)(A + KA));
  ...
  if ((now - tDetect_) >= RS_T_INTERVAL_MS) {
    ...
    for (uint8_t i = 0; i < RS_NUM_SLAVES; ++i) {
      if (online_[i] && (now - lastHB_[i]) > RS_HB_TIMEOUT_MS) {
        ...
        if (KR > 0) sendFrame(rBuf, (size_t)(R + KR));
      }
    }
  }
